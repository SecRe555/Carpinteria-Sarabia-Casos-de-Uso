...
public class Interfaz extends JFrame {
	...
	private void createCarpentryTab() {
		...
		imgLogin.setIcon(new ImageIcon(getClass().getResource("/files/img-login.jpg")));
	...
	private JPanel createEnterKeyCodePanel() {
		...
		field3.addKeyListener(new KeyAdapter() {
			public void keyTyped(KeyEvent evt) {
				...
				if (charTyped == '\n') {
					confirmKeyCode.doClick();
				} else if (charTyped == KeyEvent.VK_DELETE && field3.getText().equals("")) {
					field2.requestFocus();
				} else if (!(Character.isDigit(charTyped))) {
					Toolkit.getDefaultToolkit().beep();
					evt.consume();
		...
		field4.addKeyListener(new KeyAdapter() {
			public void keyTyped(KeyEvent evt) {
				...
				if (charTyped == '\n') {
					confirmKeyCode.doClick();
				} else if (charTyped == KeyEvent.VK_DELETE && field4.getText().equals("")) {
					field3.requestFocus();
				} else if (!(Character.isDigit(charTyped))) {
					Toolkit.getDefaultToolkit().beep();
					evt.consume();
		...
		field5.addKeyListener(new KeyAdapter() {
			public void keyTyped(KeyEvent evt) {
				...
				if (charTyped == '\n') {
					confirmKeyCode.doClick();
				} else if (charTyped == KeyEvent.VK_DELETE && field5.getText().equals("")) {
					field4.requestFocus();
				} else if (!(Character.isDigit(charTyped))) {
					Toolkit.getDefaultToolkit().beep();
					evt.consume();
